...
export const Sidebar = ({ className }) => {
    ...
    const email = useAuthStore((state) => state.email) || localStorage.getItem('user_email');
    const first_name = useAuthStore((state) => state.user_first_name) || localStorage.getItem('user_first_name') || "Doctor";
    const last_name = useAuthStore((state) => state.user_last_name) || localStorage.getItem('user_last_name') || "";
    ...
    return (
        <>
            <div className={cn("card flex flex-col gap-12 rounded-2xl p-4 border-2 border-[#7fee64] justify-between items-center mb-4 mt-4 h-fit z-5", className)}>
                <Link href='/'>
                    <div className="">
                        <h1 className="font-bold text-xl">GIST</h1>
                        <h1 className="font-bold text-xl">3DR</h1>
                    </div>
                </Link>
                    ...
                    <li>
                        <TooltipProvider delayDuration={0}>
                            <Tooltip>
                                <TooltipTrigger>
                                    <Link href='/'>
                                        <button className="card  text-white text-sm flex gap-2 p-2 rounded-full bg-[#7fee64] border-[#7fee64] text-[#1b1b1b] hover:scale-110 transition-transform duration-200">
                    ...
                    <PopoverContent className="" side="right">
                        ...
                        <BackgroundGradient className=" max-w-sm p-4">
                            <div className='flex gap-2'>
                                ...
                                {loading ? <>
                                    <ThreeDots height="80" width="80" color="#7fee64" ariaLabel="loading" /></> : <button onClick={handleLogoutFunc} className="card w-14 h-14 flex justify-center items-center text-white text-sm p-2 rounded-full bg-[#7fee64] border-[#7fee64] text-[#1b1b1b] hover:scale-110">
